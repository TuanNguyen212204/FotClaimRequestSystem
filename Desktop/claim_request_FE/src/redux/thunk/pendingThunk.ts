...
export const fetchAllClaims = createAsyncThunk(
  "pending/fetchAllClaims",
  async () => {
    const res = await axios.get(
      `https://67263146302d03037e6cb422.mockapi.io/pending`
    );
  ...
  async (id: string) => {
    if (window.confirm("Are you sure you want to delete this claim?")) {
      const res = await axios.delete(
        `https://67263146302d03037e6cb422.mockapi.io/pending/${id}`
      );
      console.log("Data", res.data);
      return id;
    }
...
export const pendingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteClaim.fulfilled, (state, action) => {
      state.listClaims = state.listClaims.filter(
